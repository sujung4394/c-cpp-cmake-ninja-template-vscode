...
4. **컴퓨터를 재부팅합니다.**
   ...
   VSCode·터미널을 재시작하는 것만으로 대부분 충분하지만, Path 변경 사항을
   탐색기(explorer.exe)나 다른 백그라운드 프로세스가 계속 옛 값으로 들고 있는
   경우가 가끔 있습니다. 특히 이번이 Path를 처음 만지는 것이거나, 6번 확인에서
   "새 터미널을 열었는데도" 여전히 안 잡힌다면 재부팅이 가장 확실합니다.
...
| 설치는 됐는데 VSCode에서만 못 찾음 | Path 등록 후 VSCode를 **완전히 종료 후 재실행**해야 반영됨. 그래도 안 되면 컴퓨터를 재부팅하세요 |
